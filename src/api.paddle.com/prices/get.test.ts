...

{
  ...
  listUrl.searchParams.set("recurring", "true");
...
{
  console.info("success not recurring");
  ...
  const listUrl = new URL("http://localhost:3001/https://sandbox-api.paddle.com/prices");
  listUrl.searchParams.set("recurring", "false");
  ...
  const listProduct2RecurringBody = await listProduct2Recurring.json();
  if (listProduct2RecurringBody.data.length !== 2) throw new Error();
  if (listProduct2RecurringBody.data[0].description !== "product1-onetime") throw new Error();
  if (listProduct2RecurringBody.data[1].description !== "product2-onetime") throw new Error();
}
