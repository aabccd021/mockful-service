...

{
  ...
  const transactionCreate = await fetch(
  ...
  if (transactionCreate.status !== 201) throw new Error();
  const transactionCreateBody = await transactionCreate.json();
  if (transactionCreateBody.data.created_at !== "2024-01-01T00:00:00.000Z") throw new Error();
  if (transactionCreateBody.data.status !== "draft") throw new Error();
  if (transactionCreateBody.data.customer_id !== customerId) throw new Error();
  if (transactionCreateBody.data.items.length !== 1) throw new Error();
  if (transactionCreateBody.data.items[0].price_id !== priceId) throw new Error();
  if (transactionCreateBody.data.items[0].quantity !== 3) throw new Error();

  const transactionId = transactionCreateBody.data.id;
  if (typeof transactionId !== "string") throw new Error();
